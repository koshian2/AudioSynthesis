...
def _audio_synthesis(query_data, filename, max_retry):
    # synthesis
    synth_payload = {"speaker": 1}
    for synth_i in range(max_retry):
        query_data["speedScale"] = 1.1
        query_data["intonationScale"] = 0.7
        query_data["volumeScale"] = 2.7
...
def render_datasets(article_data_path):
    audio_dir = "audio_zunda"
    moras_dir = "moras_zunda"
    ...
    with open("logs_zunda.txt", "w", encoding="utf-8") as fp:
        fp.write("\n".join(failed))

if __name__ == "__main__":
    render_datasets("crawl/articles_all/hokkaido.txt")
